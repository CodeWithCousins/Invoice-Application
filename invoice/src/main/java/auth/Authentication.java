...
public class Authentication {
	...
	public boolean IsValidCredentials(String token, int organizationId) throws SQLException, ParseException, JSONException
	{
		...
		if(dateGenerator.TokenExpiryValidator(json.getString("expired at"))) // Checks the token validity
		{
			...
			PreparedStatement ps = con.prepareStatement("SELECT * FROM Users WHERE token = ? and id = ?");
